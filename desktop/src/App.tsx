...
const App: React.FC = () => {
  ...
  // Load agents from Rust backend via Tauri IPC
  useEffect(() => {
    async function load() {
      try {
        // Will invoke Rust command when Tauri is ready
  ...
  const handleSend = useCallback(async () => {
    ...
    setMessages(prev => [...prev, `📋 PM Agent 已收到: "${input}"`]);
    setMessages(prev => [...prev, `🔄 SOP 管道启动: PM → Spec → Coding → CR → TDD → Acceptance → Security → DevOps → Secretary`]);
    setInput("");

    // TODO: invoke Tauri command to run the pipeline
    // const result = await invoke("run_pipeline", { prompt: input });
  }, [input]);
          ...
          <div style={styles.agentList}>
            {agents.map((agent) => (
              <div key={agent.name} style={styles.agentCard}>
                ...
                <span style={{
                  ...styles.agentModel,
                  background: agent.model === "pro" ? "#1a3a5c" : "#2d4a2d"
                }}>
        ...
        <div style={styles.center}>
          ...
          <div style={styles.bottom}>
            <div style={styles.terminal}>
              <span style={{ color: "#888", fontSize: 12 }}>$ 终端就绪 — 等待 Agent 输出...</span>
            </div>
...
const styles: Record<string, React.CSSProperties> = {
  container: {
    ...
    background: "#1e1e1e", color: "#ccc", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif",
  },
  titleBar: {
  ...
  agentsBadge: {
    ...
    background: "#3c3c3c", padding: "2px 8px", borderRadius: 10,
  },
  main: { display: "flex", flex: 1, overflow: "hidden" },
  sidebar: {
    width: 280, background: "#252526",
    borderRight: "1px solid #3c3c3c", display: "flex", flexDirection: "column",
  },
  sidebarHeader: {
    padding: "10px 14px", fontSize: 11, fontWeight: 600, textTransform: "uppercase",
    color: "#888", borderBottom: "1px solid #3c3c3c", letterSpacing: 1,
  },
  ...
  agentModel: {
    fontSize: 10, padding: "1px 6px", borderRadius: 8, color: "#ccc", flexShrink: 0,
  },
  ...
  terminal: {
    height: 120, padding: 12, background: "#1a1a1a",
    fontFamily: "'Cascadia Code', 'Fira Code', monospace", fontSize: 12,
    overflow: "auto",
  },
  inputBar: {
    display: "flex", padding: 8, gap: 8, background: "#2d2d2d",
    borderTop: "1px solid #3c3c3c",
  },
  input: {
    flex: 1, padding: "8px 12px",
    background: "#3c3c3c", border: "1px solid #555",
    color: "#ccc", borderRadius: 4, fontSize: 13, outline: "none",
  },
  sendBtn: {
